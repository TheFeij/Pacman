...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        searchedCorners = [False, False, False, False]
        return self.startingPosition, searchedCorners
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.WEST, Directions.SOUTH, Directions.EAST]:
            ...
            position, searchedCorners = state
            x, y = position
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            newSearchedCorners = searchedCorners[:]
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                index = 0
                for corner in self.corners:
                    if (nextx, nexty) == corner:
                        newSearchedCorners[index] = True
                        break
                    index += 1
                newState = ((nextx, nexty), newSearchedCorners)
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0, len(corners)):
        if not state[1][i]:
            ...
            x1 = min(x, corners[i][0])
            x2 = max(x, corners[i][0])
            y1 = min(y, corners[i][1])
            y2 = max(y, corners[i][1])
                ...
                for Y in range(y1 + 1, y2):
                    if walls[X][Y]:
                        if counter1 == 0:
                            ...
                            if abs(previousWall[0] - X) <= 1 \
                                    and abs(previousWall[1] - Y) <= 1:
                                previousWall = [X, Y]
                                counter1 += 1
                                break
                        counter1 += 1
...
def foodHeuristic(state, problem):
    ...
    for foodPosition in foodList:
        ...
        if foodGrid[foodX][foodY]:
            distance = abs(x - foodX) + abs(y - foodY)
                ...
                for Y in range(y1 + 1, y2):
                    if walls[X][Y]:
                        if counter1 == 0:
                            ...
                            if abs(previousWall[0] - X) <= 1 and abs(previousWall[1] - Y) <= 1:
                                previousWall = [X, Y]
                                counter1 += 1
                                break
                        counter1 += 1
